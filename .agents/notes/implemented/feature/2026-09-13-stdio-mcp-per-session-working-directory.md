...
The pool owns a publishing fallback connection plus one private connection per session working directory, created lazily on the first call from that directory. The fallback connection is a connection under config `cwd` and is the tool-surface authority: it runs discovery, registers the `mcp__<serverName>__<rawName>` tools, and re-syncs on the client's list-changed subscription. Private connections pass `publish: false`, so they serve calls while sharing the same `serverName` and raw names without registering a second tool surface, running discovery, or re-syncing on a list change. `PoolHandle` also implements `ServerContext`, delegating `resources` and `instructions()` to the fallback connection, so `registerServerContext` publishes one resource provider and one attributed-instructions section per plugin instance regardless of how many private connections the pool opens.
...
`packages/mcp/mcp-client/tests/mcp-client.e2e.ts` ("spawns one child per session directory and reuses it") runs the real fixture child over stdio; the `where` tool added to [`tests/fixture-server.ts`](../../../../packages/mcp/mcp-client/tests/fixture-server.ts) reports `process.cwd()` and `process.pid`, proving that two session directories get different child processes in their own directories, that a repeated directory reuses its child's pid, and that a call without a session runs in the configured fallback. The package unit suite (128 tests) passes, `tsc -b tsconfig.host.json` passes, and per-file coverage of `packages/mcp/mcp-client/src` is 100%.
